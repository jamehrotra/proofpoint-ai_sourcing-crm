...
export function AIProfileSection({ profile }: AIProfileSectionProps) {
  return (
    <section className="border border-[#e8e2d4] bg-white p-10 mb-6">
      ...
      <div className="grid grid-cols-2 gap-x-16 gap-y-9 mb-9">
      ...
      {profile.competitiveLandscape && (
        <div className="pt-8 border-t border-[#e8e2d4]">
      ...
      {profile.risks.length > 0 && (
        <div className="mt-8 pt-8 border-t border-[#e8e2d4]">
          <div className="font-mono text-[10px] uppercase tracking-[0.12em] text-[#6b6358] mb-4">Identified Risks</div>
...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <div className="font-mono text-[10px] uppercase tracking-[0.12em] text-[#6b6358] mb-2">{label}</div>
      <p className="text-[15px] text-[#1a1816] leading-[1.65]">{value}</p>
    </div>
  );
}
