interface ScanProgressProps {
  mode: 'search' | 'analyze';
  count?: number;
  urlBeingFetched?: string | null;
}

export function ScanProgress({ mode, count, urlBeingFetched }: ScanProgressProps) {
  return (
    <div className="border border-[#e8e2d4] bg-[#faf7f2] py-16 px-8 text-center">
      ...
      <p className="font-serif text-[20px] italic text-[#1a1816] mb-2">
        {urlBeingFetched
          ? 'Fetching page content and scoring thesis fit'
          : mode === 'search'
      ...
      <p className="font-mono text-[10px] uppercase tracking-[0.1em] text-[#6b6358]">
        {urlBeingFetched
          ? <>Pulling content via Tavily → You.com → Jina · ~10 to 30 seconds</>
